"""IE2 v14 · SONDA de código: ancho del diálogo 0xF0 -> 0x1A0 (22 -> 37 caracteres por línea).

Autorizada explícitamente por el usuario (parche de código en ina_main2.cro, solo inmediatos).

CRO (base: ie2/shared/capas/menus_cro/cofres = la de probe_ie2_v10):
...
Uso: python -X utf8 work/ie2/shared/capas/historial/menus_cro/v14_ancho_dialogo/apply.py
"""
...
ROOT = HERE.parents[6]
sys.path.insert(0, str(ROOT / 'work/ie2/tormenta_de_fuego/capas/dialogo/dialogo'))
import comun_ie2 as M  # noqa: E402

from capstone import CS_ARCH_ARM, CS_MODE_ARM, Cs  # noqa: E402

W = ROOT / 'work'
CRO_BASE = W / 'ie2/shared/capas/menus_cro/cofres/romfs/cro/ina_main2.cro'
BASE = W / 'shared/candidatas/probe_ie2_v10/archive.fa'
PARES = W / 'ie2/tormenta_de_fuego/capas/dialogo/dialogo/pares.json'
...
def parchear_cro():
    ...
    assert len(difs) <= 8 and all(any(a <= i < a + 4 for a, *_ in PARCHES) for i in difs), difs
...
def main():
    ...
    assert HERE.parent.name == 'v14'
    cro = parchear_cro()
    evs = eventos()
    inf = dict(
        sonda='IE2 v14 ancho del diálogo 0x1A0 (37 × 3), parche de código autorizado por el usuario',
