"""Codificador DSP-ADPCM (GC/3DS) en Python puro.
...
de 14 muestras (1 byte de predictor/escala + 7 bytes de nibbles). El decodificador está aquí solo
para comprobar la ida y vuelta (validate.py usa además vgmstream sobre el CWAV entero).
"""
...
def coeficientes(pcm):
    ...
    for z in range(8):
        ...
        for k in (1, 2):
            d = -best[z][k] * 2048.0
            if d > 0.0:
                par.append(32767 if d > 32767.0 else int(math.floor(d + 0.5)))
            else:
                par.append(-32768 if d < -32768.0 else -int(math.floor(-d + 0.5)))
