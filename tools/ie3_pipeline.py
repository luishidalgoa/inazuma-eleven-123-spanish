...
from ie123kit.ie3.comun.rom import (
    RomError, detect_kind, prepare_romfs, product_code, title_id,
)
from ie123kit.ie3.comun.ssd import (
    SSDError, group_ruby, is_ssd, parse_flat_text, parse_ssd,
)
...
class Row:
    ...
    __slots__ = ("instruction", "argument", "offset", "text", "string_id",
                 "opcode")
...
def cmd_reinsert(args):
    ...
    Sin --crecer: los bloques de evet conservan tamano y numero de registros.
    Con --crecer: los registros se agrandan lo que haga falta (entra el 100% de
    lo traducido) y se reconstruyen el .pkb y el .pkh; el numero de registros
    sigue intacto, que es de lo que cuelgan las referencias del script.
    """
    import shutil
    ...
    log.section("REINSERTAR IE3 EN archive.fa"
                + ("  (registros que crecen)" if args.crecer else "  (mismo tamano)"))
            ...
            for eid, off, size in packnum.parse_index(pkh):
                ...
                try:
                    nuevo, n, fuera = parchear_bloque(
                        pkb[off:off + size], trad[eid], crecer=args.crecer
                    )
            ...
            for eid, off, size in packnum.parse_index(epkh):
                ...
                try:
                    ...
                    if dec[:4] != b"SSD" + bytes(1):
                        continue
                    nuevo, n = parchear_ssd(dec, trad[eid], set(TEXT_OPCODES))
...
def main():
    ...
    p.add_argument("--crecer", action="store_true",
                   help="deja crecer los registros de evet (entra el 100%% de lo traducido)")
