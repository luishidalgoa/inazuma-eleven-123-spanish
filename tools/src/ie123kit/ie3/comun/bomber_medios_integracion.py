...
SCHEMA = "ie3_bomber_media_extension_v1"
...
def planificar(root: Path, base: Path, source: Path, state: dict, motor: Motor, cache: Path,
               *, decoder: Callable = decodificar) -> dict:
    """Plan cerrado a cuatro vídeos y dos destinos de canciones. Solo lee fuentes."""
...
def huellas_paquete(motor: Motor) -> dict:
    paths = [Path(__file__), Path(__file__).with_name("bomber_video_qa.py"),
             Path(__file__).parent.parent / "bomber_medios.py"]
...
def preparar(root: Path, base: Path, source: Path, output: Path, *, motor: Motor | None = None,
    ...
    plan = planificar(root, base, source, state, motor, cache, decoder=decoder)
    ...
    revision = {
        "schema": SCHEMA, "label": "Fase 5 + medios Bomber — pendiente de validación manual",
...
def verificar(root: Path, candidate: Path, *, motor: Motor | None = None) -> dict:
    ...
    if len(plan["video"]) != len(expected_targets) or {r["target"] for r in plan["video"]} != expected_targets:
        raise ValueError("El plan de vídeos no es la lista cerrada de cuatro destinos")
    expected = {}
    for row in plan["video"]:
        ...
        comparar_pareja(row["qa_jp"], row["qa_es"])
    ...
    if len(plan["audio"]) != len(expected_audio) or {r["target"] for r in plan["audio"]} != expected_audio:
        raise ValueError("Plan de canciones diferente de los dos destinos auditados")
