"""Construye Fase 5 + medios Bomber sin reemitir texto.
v1.1: política explícita para conservar el opening JP como pareja completa.
El modo estricto sigue rechazando cualquier diferencia de fotogramas.
...
DEFAULT_OUTPUT = "work/ie3/shared/candidatas/spark_bomber_ogre_fase5_medios_v11"
DEFAULT_ROM = "work/build/inazuma123_fase5_bomber_v11.3ds"
...
def ejecutar(root: Path, args) -> None:
    ...
    print("No se cambian diálogos, UI, fuentes, encoder, CRO ni guardados.", flush=True)
    print(f"Política de opening: {args.opening}", flush=True)
    if args.opening == "conservar-base":
        print("ALCANCE PARCIAL: el opening (vídeo y canción) se conserva en japonés.\n",
              flush=True)
    ...
    if output.exists():
        ...
        if rev.get("opening_policy") != args.opening:
            raise ValueError("La política elegida difiere de la candidata existente; usa otra salida")
        ...
        result = preparar(root, base, source, output, opening=args.opening)
    ...
    if result.get("media_pending"):
        print("OPENING BOMBER PENDIENTE: se mantienen vídeo JP y canción JP.")
        for item in result["media_pending"]:
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--opening", choices=("estricto", "conservar-base"), default="estricto",
                        help="estricto exige igualdad; conservar-base NO inserta el vídeo ni la canción op00b")
